...
- The default read URL is the backend media route `/api/v1/media/avatars/{avatar_id}`. Deployments may configure `ROUNDTABLE_AVATAR_MEDIA_BASE_URL` to return absolute backend media URLs, or `ROUNDTABLE_AVATAR_PUBLIC_BASE_URL` for CDN/public-bucket direct reads; writes still go through the backend.
