...
- The default read URL is the backend media route `/api/v1/media/avatars/{avatar_id}`. Deployments may configure `ROUNDTABLE_AVATAR_MEDIA_BASE_URL` to return absolute backend media URLs. Direct CDN/public-bucket URLs require explicit `ROUNDTABLE_AVATAR_DIRECT_PUBLIC_URLS=true` plus `ROUNDTABLE_AVATAR_PUBLIC_BASE_URL`; otherwise every storage mode uses the backend media route.
- Docker Compose enables the local avatar store by default and persists objects in the `roundtable-avatar-data` volume.
